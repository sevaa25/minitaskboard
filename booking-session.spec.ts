...
test.describe('Booking a session / entering wait list', () => {
  ...
  test('Couples Therapy', async ({page}) => {
    await page.getByRole('button', {name: "Couples Therapy"}).click();
    
    await page.getByRole("button", {name: "Search"}).click();
    await page.getByPlaceholder("Search").fill("John Wick");
    await page.getByRole('button', {name: "Book a session"}).click();
    await expect(page).toHaveURL("https://staging.wellbee.pl/en/google.pl");

    const errorMessage =  page.getByText("Ups!");
    await expect(errorMessage).toBeVisible();

  })

  test('Children/young people', async ({page}) =>{
    await page.getByRole('button', {name : "Children/young people"}).click();
    await page.getByRole('button', {name: "Parental consultations"}).click();

    await expect(page).toHaveURL("https://staging.wellbee.pl/en/znajdz-terapeute/dzieci-mlodziez/konsultacje-rodzicielskie/styl1")
    await page.getByRole('button').nth(1).click();
    await expect(page).toHaveURL("https://staging.wellbee.pl/en/znajdz-terapeute/dzieci-mlodziez/konsultacje-rodzicielskie/styl2")
    await page.getByRole('button').nth(1).click();

    await page.pause();

    await page.getByRole("button", {name: "Search"}).click();
    await page.getByPlaceholder("Search").fill("Natalia Pilarska- Długie");
    await page.getByRole('button', {name: "Join the waiting list"}).click();
